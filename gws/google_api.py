...
def gmail_draft(args):
    if _gws_binary():
        ...
        msg = {"raw": raw}
        if args.thread_id:
            msg["threadId"] = args.thread_id
        result = _run_gws(["gmail", "users", "drafts", "create"], params={"userId": "me"}, body={"message": msg})
        print(json.dumps({"status": "draft_created", "id": result.get("id", ""), "note": "DRAFT ONLY - not sent"}, indent=2))
        return
    service = build_service("gmail", "v1")
    message = MIMEText(args.body, "html" if args.html else "plain")
    message["To"] = args.to
    ...
    if args.thread_id:
        msg["threadId"] = args.thread_id
    result = service.users().drafts().create(userId="me", body={"message": msg}).execute()
    print(json.dumps({"status": "draft_created", "id": result.get("id", ""), "note": "DRAFT ONLY - not sent"}, indent=2))
